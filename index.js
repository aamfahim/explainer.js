...
program.argument('<files...>', 'path of the files or directories to process')
    .action(async (files, options) => {
        try {
            const { apiKey, baseURL, temperature, model, outputFile, tokenUsage } = ExtractOptions(options, TomlChecker());
            ...
            const responses = await Promise.all(resolvedFiles.map(async (file) => {
                return await ProcessFileWithProvider(Groq, BuildFilePrompt(file), model, temperature);
            }));
